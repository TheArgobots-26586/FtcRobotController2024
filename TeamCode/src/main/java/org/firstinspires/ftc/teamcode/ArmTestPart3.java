...
@TeleOp(name="Arm Test Part 3", group="TeleOp")

public class ArmTestPart3 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if(extending && curExtenderPos + 30 <= 1300) {
                curExtenderPos += 30;
                extender.setTargetPosition((int) curExtenderPos);
                ((DcMotorEx) extender).setVelocity(1000);
                extender.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            }
            else if(collapsing && curExtenderPos - 30 >= 0) {
                curExtenderPos -= 30;
                extender.setTargetPosition((int) curExtenderPos);
                ((DcMotorEx) extender).setVelocity(1000);
                extender.setMode(DcMotor.RunMode.RUN_TO_POSITION);
